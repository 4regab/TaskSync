...
test('pin login succeeds when TASKSYNC_E2E_PIN is provided', async ({ page }) => {
    test.skip(!/^\d{4,6}$/.test(e2ePin), 'Set TASKSYNC_E2E_PIN=4..6 digit PIN to run this test');
    ...
    await expect(page.locator('.remote-header-title')).toHaveText(/TaskSync/i, { timeout: 15000 });
});
