...
function generateSharedConstants() {
    const source = fs.readFileSync(
        path.join(__dirname, 'src', 'constants', 'remoteConstants.ts'), 'utf8',
    );
    ...
    function extractNum(name) {
        ...
        if (!m) throw new Error(`Failed to extract ${name} from remoteConstants.ts`);
        return Number(m[1]);
    }
    ...
    if (!timeoutMatch) throw new Error('Failed to extract RESPONSE_TIMEOUT_ALLOWED_VALUES');
    const timeoutValues = timeoutMatch[1].split(',').map(s => s.trim()).filter(Boolean).join(', ');
...
function buildWebview() {
    ...
    for (const file of WEBVIEW_FILES) {
        const content = fs.readFileSync(path.join(WEBVIEW_SOURCE_DIR, file), 'utf8');
        body += content;
